    ...
    if (result) {
      ...
      return axios.get(searchUrl)
        .then(response => {
          ...
          // Save the Wikipedia API response in Redis store, and keep for 3600 seconds = 1 hour
          // client.setex(`wikipedia:${query}`, 3600, JSON.stringify({ source: 'Redis Cache', ...responseJSON, }));
          // changed to 300 seconds = 5 minutes
          client.setex(`wikipedia:${query}`, 300, JSON.stringify({ source: 'Redis Cache', ...responseJSON, }));
